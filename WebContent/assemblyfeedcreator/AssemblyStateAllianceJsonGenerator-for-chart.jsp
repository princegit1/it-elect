...
<%
		...
		String won="0";
		String lead="0";
		String allinaceDeatils[]={"KERALA","PUDUCHERRY","TAMIL NADU","WEST BENGAL","ASSAM"};
		...
		try {
			...
			stmt = conn.createStatement();		
					...
					String Query = "SELECT e.state, e.party, e.previouswon, e.currentwon AS won, e.currentleading AS lead, e.gainloss AS GAINLOSS, pm.party_color_code,pm.abbr_h AS PARTY_h FROM election_seat_count_master e, party_master pm WHERE e.assembly=1 AND e.state='"+allinaceDeatils[i]+"' AND e.active='1' AND TRIM(LOWER(pm.abbr))=TRIM(LOWER(e.party))  ORDER BY PRIORITY";
					//String Query = "SELECT sp.`STATE`,SUM(sp.WON) AS won,SUM(sp.LEAD) AS lead,SUM(sp.WON)+SUM(sp.LEAD) AS 'Lead+Won',sp.party,pm.`PARTY_COLOR_CODE` FROM `statewise_partydetails`sp,`party_master` pm WHERE sp.state='"+allinaceDeatils[i]+"' AND sp.active='1' AND sp.ASSEMBLY='1' AND sp.party=pm.ABBR AND sp.STATUS IN('LEADING','WON') GROUP BY sp.party";
	...
	//out.println("@@@@@@@@@@@@@@"+Query);
		...
		//assam 126
			...
			if(alliance.equals("KERALA")){
		totalResult=140 ;
		
		}else if(alliance.equals("PUDUCHERRY")){
		totalResult=30;
		
		}else if(alliance.equals("TAMIL NADU")){
		totalResult=234;
		
		}else if(alliance.equals("WEST BENGAL")){
		totalResult=294;
		
		}else if(alliance.equals("ASSAM")){
		totalResult=126;
...
//out.println("@@@@-->"+totalResult1);
            ...
            if(parseData != null && parseData.size() > 0)
            {
			 ...
			 sb.append("]}})");
			 xmlData=sb.toString();
             fileName=alliance.replaceAll(" ", "-").replaceAll("&", "and").toLowerCase().replaceAll("%20", "").replaceAll(":", "")+"-assembly-alliance-chart";
