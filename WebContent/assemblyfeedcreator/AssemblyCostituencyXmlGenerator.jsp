...
<%
		...
		try {
			...
			String Query = "select STATE_NAME,TOTAL_SEATS_2014 from state_master where STATE_ID in ('S11','S27') order by STATE_ID_2014";
		...
		if (parseData != null && parseData.size() > 0) {
			...
			for (int i = 0; i < parseData.size(); i++) {
				...
				if("JHARKHAND".equalsIgnoreCase(name)){
				constituencyId=Integer.parseInt(cid)-27000;
				}else if("JAMMU&KASHMIR".equalsIgnoreCase(name)){
				constituencyId=Integer.parseInt(cid)-9000;
				}

				sb.append("<CONSTITUENCY ID=" + '"' + constituencyId
						+ '"' + " NAME=" + '"'
		...
		String xmlData=sb.toString();
         ...
         String fileName=name.replaceAll(" ", "_").replaceAll("&", "and").toLowerCase().replaceAll("%20", "").replaceAll(":", "")+"_assembly_constituency_tally";;
        //generateXmlFile(xmlData, fileName); 
        generateXmlFile(xmlData, fileName,xmlPath,".xml");      
			}
	...
	public List getParsedNewConstituencyData(String stateName) {
		...
		try {
			...
			String Query = "select 	CID,STATE_NAME,CNAME,SEATING_MP,SEATING_PARTY_NAME,WINING_MP,WINING_PARTY,STATUS,ASSEMBLY from constituency_master where STATE_NAME='"
					+ stateName + "' and ASSEMBLY='1' and ACTIVE='1' ORDER BY CID";
